...
class RicoNeitzel_VertNav_Block_Navigation extends Mage_Catalog_Block_Navigation
{
	...
	public function getActiveCategories()
	{
		...
		while($category->getParentCategory()->getId() != '2') 
		{	
			$category = $category->getParentCategory();
			$activeCategories[$category->getId()] = 1;
			//print_r($activeCategories); exit();
		}
		
		if(Mage::registry('current_category')->hasChildren()) {
			$activeCategories[Mage::registry('current_category')->getId()] = 1;
		}
    ...
    public function drawOpenCategoryItem( $category, $level = 0, array $levelClass = NULL )
    {
        ...
        $productCount = '';
        if( $this->displayProductCount() && $level > 0 ) {
		...
		$activeClass = '';
		if(isset($activeCategories[$category->getId()]))
		$activeClass .= 'class="vertnav-active"';
		
		$onclick = '';


        // indent HTML!
        $html[ 1 ] = str_pad( "", ( ( $level * 2 ) + 4 ), " " ) . '<a '.$onclick.'  '.$activeClass.' href="' . $this->getCategoryUrl( $category ) . '"><span>' . $this->htmlEscape( $category->getName() ) . '</span>' . $productCount . '</a>';
