...
class Fishpig_AttributeSplashPro_PageController extends Mage_Core_Controller_Front_Action
{
	...
	protected function _initViewActionLayout(Fishpig_AttributeSplashPro_Model_Page $page)
	{
		...
		$titleStr = (count($title) > 0) ? ' '.implode($title) : '';

		$pageTitle = ''; $pageDesc = ''; $pageKey = '';
		if ($pageNum > 1) {
			$pageTitle = 'Страница '.$pageNum.' - '; $pageDesc = ' Страница '.$pageNum; $pageKey = ', страница '.$pageNum;
		}
		...
		if (($headBlock = $layout->getBlock('head')) !== false) {
			if ($page->getMetaDescription()) {
				$headBlock->setDescription($page->getMetaDescription().$titleStr.$pageDesc);
			}
			
			if ($page->getMetaKeywords()) {
				$headBlock->setKeywords($page->getMetaKeywords().$titleStr.$pageKey);
			}
			...
			if (($title = $page->getPageTitle()) !== '') {
				$headBlock->setTitle($pageTitle.$title.$titleStr);
			}
			else {
	...
	protected function _getPage()
	{
		...
		$page = Mage::getModel('splash/page')
			...
			->load($this->getRequest()->getParam('id', $this->getRequest()->getParam('page_id')));
		
		Mage::register('splash_page', $page->getId() ? $page : false);
