...
class Fishpig_Wordpress_Model_Observer_Adminhtml_SaveProductAssociations extends Fishpig_Wordpress_Model_Observer_Adminhtml_SaveAssociationsAbstract
{
    ...
    {
        try {
            if ($this->_getProduct() && $this->_getStoreIds()) {
    ...
    protected function _deleteCurrentAssociations($type)
    {
        ...
        foreach ($this->_getStoreIds() as $storeId) {
            $this->_getResource()->getConnection('core_write')
                ->delete($table, $this->_getResource()->getConnection('core_write')->quoteInto('product_id=? AND store_id=' . $storeId, $this->_getProduct()->getId()));
        }
    }
