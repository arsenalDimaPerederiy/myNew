...
class ManaPro_FilterSeoLinks_Model_Observer extends Mage_Core_Helper_Abstract {
    ...
    protected function _addAppliedFiltersToCategoryTitle($head, $appliedFilters, $page) { 
        if (count($appliedFilters) || $page && $page > 1) {
			...
			/* Inject meta data to mana filters */
			$_appliedFiltersArray = array();
			foreach ($appliedFilters as /* @var $item Mana_Filters_Model_Item */$filterItem) {
			...
			foreach ($splashPageCollection as $splashPage) {
				...
				$priceData = unserialize($splashPage->getPriceFilters());
				$priceKey =  key($priceData);
				$_priceFilter = false;
				...
				foreach($_appliedFiltersArray as $item) {
					...
					if($data[0] == 'price') {
						...
						//echo $min."-".$priceData[0]."+".$max."-".$priceData[1]."=";
						if(($min == $priceData[0]) && (($max == $priceData[1]) || (($max == 9000) && ($priceData[1] > 400)))) {
							$_splashPageMatch++;
							$_priceFilter = true;
						}
					}
			...
			if($currentSplashPage) {
				$head
					->setTitle($currentSplashPage->getPageTitle())
					->setData('keywords', $currentSplashPage->getMetaKeywords())
					->setData('description', $currentSplashPage->getMetaDescription());
			} else {
				$head
