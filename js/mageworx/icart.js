...
updateLinks:function(){
	...
	/*jQuery.get("http://a-shop.ua/checkout/icart/topcartview", {}, function(data){ 
		iCart.replaceBlock($$('div.topcart', 'div.mini-cart'), data);
	});*/
...
setLocation:function(url){
    if(url.match(/\/checkout\/i?cart\/add\//)){
        url=url.replace('/cart','/icart');
		temp = url.split("product/");
		id = temp[1].split("/");
		color = jQuery('#the-' + id[0] + ' div.coverlay').attr("id");
