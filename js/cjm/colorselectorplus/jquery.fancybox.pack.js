...
(function (window, document, $, undefined) {
	...
	jQuery.extend(F, {
		...
		_setDimension: function () {
			...
			origWidth  = 845; //isPercentage(width)  ? (viewport.w - wSpace) * getScalar(width)  / 100 : width;
			origHeight = 300; //isPercentage(height) ? (viewport.h - hSpace) * getScalar(height) / 100 : height;
				...
				if (current.autoHeight && iframe.data('ready') === 1) {
					try {
						if (iframe[0].contentWindow.document.location) {
							...
							origHeight = 300; //body.outerHeight(true);
						}
		...
		_getPosition: function (onlyAbsolute) {
			...
			rez.top  = 0 ; //getValue(Math.max(rez.top,  rez.top  + ((viewport.h - height) * current.topRatio)));
			rez.left = 0; //getValue(Math.max(rez.left, rez.left + ((viewport.w - width)  * current.leftRatio)));
