...
//	    		/* @var $reader Mana_Core_Model_Html_Reader */ $reader = Mage::getModel('mana_core/html_reader')->setSource($html);
//	    		/* @var $parser ManaPro_FilterAjax_Model_Marker */ $parser = Mage::getModel('manapro_filterajax/script_extractor', array(
//	    			'reader' => $reader,
//	    		));
//	    		try {
//	    			$parser->parseContent();
//	    			$updates['.mb-'.str_replace('.', '-', $blockName)] = $parser->getFilteredOutput();
//	    			$script .= $parser->getExtractedScripts();
//	    		}
//	    		catch (Exception $e) {
//	    			Mage::log($e->getMessage()."\n\n", Zend_Log::WARN, 'content-parser.log');
//	    			$updates['.mb-'.str_replace('.', '-', $blockName)] = $html;
//	    		}
                $updates['.mb-' . str_replace('.', '-', $blockName)] = $html;
    		}
    	}
    	...
    	if ($this->getLayout()->getBlock('head')) {
    	    $result['title'] = $this->getLayout()->getBlock('head')->getTitle();
        }
