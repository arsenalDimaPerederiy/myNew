					...
					foreach($options as $option):
						...
						foreach($images as $key => $image):
							if($product_base['morev'][$key] == $value):
								...
								$temp_html .= '<div class="item"><a href="'.$product_popup.'" class="cloud-zoom-gallery fancybox-more"  onclick="$(\'image\').src = this.href; $(\'cloudZoom\').href = this.href; return false;"><img src="'.$product_thumb.'"  alt="'.$product_label.'" title="'.$product_label.'" /></a></div>';
								$counter++;
							endif;
						endforeach;
					...
					foreach($options as $option):
						...
						foreach($images as $key => $image):
							if($product_base['morev'][$key] == $value):
								...
								$product_gallery = Mage::helper('colorselectorplus')->getGalUrl($product_base['id'][$key], $_product->getId());
								$temp_html .= '<div class="item"><a href="'.$product_image.'" rel="popupWin:\''.$product_gallery.'\', useZoom: \'fancyboxImage\', smallImage: \''.$product_image.'\'" class="cloud-zoom-gallery fancybox-more"  onclick="$(\'image\').src = this.href; $(\'cloudZoom\').href = this.href; return false;"><img src="'.$product_thumb.'"  alt="'.$product_label.'" title="'.$product_label.'" /></a></div>';
								$counter++;
							endif;
						endforeach;
	...
	public function findColorImage($value, $arr, $key, $type)
	{
        ...
        public function getIconListHtml($_product, $_currentColor)
        {
			...
			if(count($usedOptions)>1){
				foreach($usedOptions as $key => $option) {
					...
					if(in_array($option['value_index'], $_currentColor))
					{
						...
						$html .= '<li><img id="prod'.$_product->getId().'swatch'.$option['value_index'].'" src="'.$this->getSwatchUrl($option['value_index']).'"/></li>';
					}
			...
			foreach($_currentColor as $key=>$color){
				...
				$html = '<li><img id="prod'.$_product->getId().'swatch'.$color.'" src="'.$this->getSwatchUrl($color).'"/></li>'.$html;
