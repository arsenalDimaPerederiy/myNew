...
class Mana_Filters_Block_View extends Mage_Catalog_Block_Layer_View {
    protected function _construct()
    {
		...
		//print_r($this->getLayer()); exit();
		if(Mage::registry('splash_page')){
		    $categoryIds = Mage::registry('splash_page')->getCategoryIds();
			...
			foreach(Mage::registry('splash_page')->getOptionFilters() as $_code => $_filter){
				...
				$_request = '';
				$_counter = 1;
				foreach($_filter['value'] as $_values){
...
//print_r($filterOptions->getCode());
	...
	/*	print_r($this->getData('filters'));
	*/
		
//print_r($this->getLayer()->getState()->getData('filters'));
