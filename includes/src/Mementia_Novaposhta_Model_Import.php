<?php
class Mementia_Novaposhta_Model_Import
{
    ...
    /**
     * @throws Exception
     * @return Mementia_Novaposhta_Model_Import
     */
    public function runWarehouseAndCityMassImport()
    {
        ...
        try {
            /** @var $apiClient Mementia_Novaposhta_Model_Api_Client */
    ...
    protected function _importCities(SimpleXMLElement $cities)
    {
        ...
        if (count($cities) > 0) {
            ...
            foreach ($cities as $chunk) {
                $sql_header = 'INSERT INTO `novaposhta_city` (' . implode(', ', array_keys($chunk[0])) . ') VALUES ';
                $values = "";
                $i = 0;
                foreach ($chunk as $cityToInsert) {
                    $values .= '("' . implode('", "', $cityToInsert) . '"), ';
                    if($i > 30) {
    ...
    protected function _importWarehouses(SimpleXMLElement $warehouses)
    {
        ...
        if (count($warehouses) > 0) {
            ...
            foreach ($warehouses as &$chunk) {
                $sql_header = 'INSERT INTO `novaposhta_warehouse` (' . implode(', ', array_keys($chunk[0])) . ') VALUES ';
                $values = "";
                $i = 0;
                foreach ($chunk as $warehouseToInsert) {
                    $values .= '("' . implode('", "', $warehouseToInsert) . '"), ';
                    if($i > 10) {
                    	$sql = $sql_header.$values;
                    	$sql = trim($sql, ', ');
                    	$sql .= ' ON DUPLICATE KEY UPDATE ';
